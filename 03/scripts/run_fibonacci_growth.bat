@echo off
:: Fibonacci-Based Organic Growth (§3.3 · Fig 10)
cd /d "%~dp0"
set BLENDER_EXE=
for %%B in (
...
if exist "%~dp0_scripting_startup.blend" (
    start "" %BLENDER_EXE% "%~dp0_scripting_startup.blend" --python "%~dp0fibonacci_growth.py"
) else (
    start "" %BLENDER_EXE% --python "%~dp0fibonacci_growth.py"
)

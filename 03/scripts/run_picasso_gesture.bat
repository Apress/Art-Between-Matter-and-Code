@echo off
:: Picasso's Procedural Gesture (§3.6.2)
cd /d "%~dp0"
set BLENDER_EXE=
for %%B in (
...
if exist "%~dp0_scripting_startup.blend" (
    start "" %BLENDER_EXE% "%~dp0_scripting_startup.blend" --python "%~dp0picasso_gesture.py"
) else (
    start "" %BLENDER_EXE% --python "%~dp0picasso_gesture.py"
)

@echo off
:: Hollow and Lattice Structures (§3.6.1.3 · Fig 16)
cd /d "%~dp0"
set BLENDER_EXE=
for %%B in (
...
if exist "%~dp0_scripting_startup.blend" (
    start "" %BLENDER_EXE% "%~dp0_scripting_startup.blend" --python "%~dp0lattice_structures.py"
) else (
    start "" %BLENDER_EXE% --python "%~dp0lattice_structures.py"
)

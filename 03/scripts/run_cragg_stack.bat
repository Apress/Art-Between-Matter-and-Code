@echo off
:: Tony Cragg: Stack — Stratified Torsional Surfaces (§3.6.3.1 · Fig 20)
cd /d "%~dp0"
set BLENDER_EXE=
for %%B in (
...
if exist "%~dp0_scripting_startup.blend" (
    start "" %BLENDER_EXE% "%~dp0_scripting_startup.blend" --python "%~dp0cragg_stack.py"
) else (
    start "" %BLENDER_EXE% --python "%~dp0cragg_stack.py"
)

...
:found
echo.
echo Blender will open with scan_cleanup.py loaded.
echo   1. File ^> Import ^> OBJ / FBX / PLY
echo   2. Click the mesh in the 3D viewport to select it
echo   3. Press Alt+P to run the cleanup
echo.
if exist "%~dp0_scripting_startup.blend" (
    start "" %BLENDER_EXE% "%~dp0_scripting_startup.blend" --python "%~dp0scan_cleanup.py"
) else (
    start "" %BLENDER_EXE% --python "%~dp0scan_cleanup.py"
)

#!/usr/bin/env bash
# ============================================================
# run_scan_cleanup.sh  —  macOS / Linux launcher
# Art Between Matter and Code · Gianpiero Moioli · Apress 2025
# Runs scan_cleanup.py in Blender 5.x (or any available version)
# Usage: chmod +x run_scan_cleanup.sh && ./run_scan_cleanup.sh
# ============================================================

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PYTHON_SCRIPT="$SCRIPT_DIR/scan_cleanup.py"
...
if [ -z "$BLENDER_BIN" ]; then
    ...
    echo "or set: export BLENDER_BIN=/path/to/blender && ./run_scan_cleanup.sh"
    exit 1
fi

echo "Using Blender: $BLENDER_BIN"
echo "Running: scan_cleanup.py"
echo "--------------------------------------------"
